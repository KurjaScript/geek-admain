      ...
      <el-dropdown @command="handleTags">
        ...
        <template #dropdown>
          <el-dropdown-menu size="small">
            <!-- command 属性：派发到 command 回调函数的指令参数 -->
...
export default {
  setup() {
    ...
    const closeTags = (index) => {
      ...
      const item = tagsList.value[index] ? tagsList.value[index] : tagsList[index - 1];
      if (item) {
    ...
    const handleTags = (command) => {
      command === "other" ? claseOther() : closeAll();
    };
