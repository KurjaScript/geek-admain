<template>
  <div>
      ...
      >
        ...
        <el-table-column prop="status" label="状态" align="center">
          <template #default="scope">
            <el-tag
              :type="
                scope.row.status === 1 || scope.row.status === true ? 'success' : 'danger'
              "
              >{{
                scope.row.status === 1 || scope.row.status === true ? "成功" : "失败"
              }}</el-tag
            >
          </template>
        </el-table-column>
    ...
    <!-- 新增弹出框 -->
    <el-dialog
      :title="titleValue === true ? '编辑用户' : '新增用户'"
      v-model="addVisible"
      width="45%"
    >
      <el-form
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button v-if="titleValue === false" type="primary" @click="saveNewForm"
            >确定</el-button
          >
          <el-button v-if="titleValue === true" type="primary" @click="saveEdit()"
            >确 定？</el-button
          >
        </span>
      </template>
...
export default {
  ...
  setup() {
    ...
    function handleSearch(queryString) {
      ...
      tableData.value.map((item) => {
        if (
          ...
          item.id.toString().indexOf(query.id) !== -1
        ) {
    ...
    const saveNewForm = () => {
      ...
      ruleForms.value.validate((valid) => {
        if (valid) {
          ...
          addVisible.value = false;
          query.pageTotal = tableData.value.length;
          miniTable.value = tableData.value.slice(
    ...
    const saveEdit = () => {
      ruleForms.value.validate((valid) => {
        if (valid) {
          ...
          miniTable.value[idx].create_time = formatDate(
            ruleForm.create_date,
            ruleForm.create_time
          );
          miniTable.value[idx]["update_time"] = formatDate(
            ruleForm["update_date"],
    ...
    const togglePagination = (page) => {
      ...
      let endNum =
        query.pageSize * page > query.pageTotal ? query.pageTotal : query.pageSize * page;
      miniTable.value = [];
